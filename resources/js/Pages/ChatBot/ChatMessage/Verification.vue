<template>
    <div class="max-w-[70%]">
        <div
            class="px-2 py-2 pb-2 rounded-lg border bg-[#F2F4F5] text-center w-full"
        >
            ...
            <p class="font-krub-medium text-[11px] mb-1">
                ...
                email address {{ message.message.email }}
            </p>
            <input
                type="number"
                maxlength="4"
                ...
                placeholder="0000"
                :readonly="disable || message.complete_booking"
                v-model="otp"
            ...
            <button
                @click="resendOtp"
                class="font-krub-medium text-[12px] mt-2"
                v-if="!countdown && !disable && !message.complete_booking"
            >
                Resend
            </button>
        </div>
...
const props = defineProps(["message", "disable"]);
...
const resendOtpInterval = () => {
    const token = props.message.message.token;
    countdown.value = "1m59s";
    if (intervalId.value) {
        clearInterval(intervalId.value);
    }
    const intervalDuration = parseInt(
        context.cookie.get(`otp-interval-second-verification-bot-email`) || "120"
    );
    ...
    intervalId.value = setInterval(() => {
        ...
        context.cookie.set(
            `otp-interval-second-verification-bot-email`,
            (minutes * 60 + seconds).toString()
        );
        if (timeLeft > 0) {
            countdown.value = `${minutes}m${seconds}s`;
        } else {
            context.cookie.set(`otp-interval-second-verification-bot-email`, "0");
            countdown.value = "";
            clearInterval(intervalId.value);
        }
    }, 1000);
...
const resendOtp = () => {
    error.value = "";
    otp.value = "";
    context.cookie.del(`otp-interval-second-${props.message.message.token}`);
...
onMounted(() => {
    if (!props.disable && !props.message.complete_booking) {
        console.log('start')
        resendOtpInterval();
    }
});
